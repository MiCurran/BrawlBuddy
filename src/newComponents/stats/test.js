...
function Stats() {
  ...
  const [legends, setLegends] = useState([
    { legend_id: 18, legend_name_key: "test" },
  ]);
  ...
  const updateUser = () => {
    axios
      .get(`http://brawlbuddyapi.herokuapp.com/players/${brawlhalla_id}`) //so we actually want to pass something like players/id to get that response
      .then((response) => {
    ...
    axios
      .get(
        `https://api.brawlhalla.com/player/${brawlhalla_id}/ranked?api_key=${apiKey}`
      ) //so we actually want to pass something like players/id to get that response
      .then((rankedResponse) => {
        ...
        <div id="stats">
          ...
          {isLoading ? (
            ...
            <div>
              ...
              <div
                className="row my-3 rank-row d-flex justify-content-center text-center "
                key={brawlhalla_id}
              >
                ...
                <div className="col-12 text-dark">
                  Win Rate: {((wins / games) * 100).toFixed(0) + "%"}
                </div>
                <div className="col-12 text-dark">
                  Elo Rating: {rating && tier}
                </div>

                <div className="col-12 text-dark">
                  Daily Elo Trend: {dailyTrend}
                </div>
              </div>
              <div className="row mx-5">
                {legends.map((item) => (
                  <div
                    className=" col-12 my-3 text-dark legendCard"
                    key={item.legend_id}
                  >
                    <div className="cardImg">
                      <img
                        src={
                          process.env.PUBLIC_URL + `${item.legend_name_key}.png`
                        }
                        alt={`${item.legend_name_key}.png`}
                      ></img>
                    </div>
                    <div className="col text-dark" id="legendName">
                      <strong>{setNameCaps(item.legend_name_key)}</strong>
                    </div>
                    <label htmlFor="level">
                      {(item.xp_percentage * 100).toFixed(0) + "%"}
                    </label>
                    <progress
                      id="level"
                      value={(item.xp_percentage * 100).toFixed(0)}
                      max="100"
                    ></progress>
