...
function Legends(props) {
  ...
  const setDisplay = () => {
    //sets the visibility of the legend component
    setDisplayLegend(!displayLegend);
  };

  if (!props.legends) {
    //when loading we display this
    return (
      <div>
        <p>no legends</p>
      </div>
  ...
  //can do on hover show a tool tip with info and say click ffor more
    ...
    if (displayLegend === false) {
      //if we havent chosen to display a specific legend yet we do render an image of each legend
                ...
                return (
                  <div>
                    <img
                      ...
                      src={`${process.env.PUBLIC_URL}/${legend.legend_name_key}.png`}
                      alt={`${legend.legend_name_key}.png`}
                    ></img>
                  </div>
                );
              })}
      ...
      return (
        <div className="legends">
          ...
          <div>
            <Button id="escapebtn" variant="primary" onClick={setDisplay}>
              X
            </Button>{" "}
          </div>
        </div>
